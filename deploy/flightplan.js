...
plan.local('deploy', local => {
  ...
  const repoStatus = local.exec('test -z "$(git status --porcelain)"', {failsafe: true});
  ...
  const remoteRefHash =  local.git(`ls-remote origin -h refs/heads/${branchName} | cut -f1`).stdout.trim();
  ...
  if (branchName != plan.runtime.hosts[0].branch) {
    local.log(`You can't deploy anything else than ${plan.runtime.hosts[0].branch} to ${plan.runtime.target}.`);
    plan.abort();
  }

  user = local.exec('git config user.name');
  local.log('Run templates build');
  local.exec(`cd ../templates && npm run build -- ${args.minify ? '' : ' --no-minification'} ${args.critical ? '' : ' --no-critical-css'}`);
...
plan.remote('deploy', remote => {
  ...
  linkedAssets.forEach(asset => {
    remote.exec(`cd ${remote.runtime.projectRoot}/releases/${versionDir}/bedrock/web/app/themes/${themeName}/static/ && rm -Rf ${asset.split('/').pop()} && ln -snf ${remote.runtime.projectRoot}/releases/${versionDir}/${asset}`);
  });
  remote.log('Link all the things');
  linkedFiles.concat(linkedDirs).forEach(link => {
    const folder = link.split('/',-1).reduce((pre, cur, i, array) => i < array.length - 1 ? `${pre}/${cur}` : `${pre}/`);
    remote.exec(`cd ${remote.runtime.projectRoot}/releases/${versionDir}/${folder} && ln -snf ${remote.runtime.projectRoot}/shared/${link}`);
  });
  remote.log('Point to current version');
  remote.exec(`cd ${remote.runtime.projectRoot} && rm -f current && ln -snf ${remote.runtime.projectRoot}/releases/${versionDir} current`);
  ...
  remote.exec(`curl -X POST --data-urlencode 'payload={"channel": "#deployments", "username": "deploybot", "text": "${user.stdout} has just deployed *${project}* project to <http://${remote.runtime.host}|its *production* server>", "icon_emoji": ":shipit:"}' https://hooks.slack.com/services/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`);
});
